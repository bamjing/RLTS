...
class Model(object):
    ...
    def CNN(self, scope, candlestate, volume_percent, time, portfolio_state, action_space, phase):
        ...
        with tf.variable_scope(scope):
            ...
            candlestate_layer = tf.layers.dense(candlestate_layer, 16, tf.nn.elu, kernel_initializer=initializer)
            ...
            volume_percent_layer = tf.layers.dense(volume_percent_layer, 16, tf.nn.elu, kernel_initializer=initializer)
            ...
            time_layer = tf.layers.dense(time_layer, 16, tf.nn.elu, kernel_initializer=initializer)
            ...
            whole_state_layer = tf.layers.dense(whole_state_layer, 32, tf.nn.elu, kernel_initializer=initializer)
            ...
            join_market_portfolio = tf.concat([whole_state_layer, portfolio_state_layer], axis=2)

            #########################################################################################################
            #########################################################################################################

            predictedValue_layer = tf.contrib.layers.batch_norm(join_market_portfolio, center=False, scale=False, is_training=phase)
